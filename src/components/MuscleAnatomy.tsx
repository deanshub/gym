...
interface MuscleAnatomyProps {
	selectedMuscles?: $Enums.MuscleGroup[];
	onMuscleSelect?: (muscle: $Enums.MuscleGroup) => void;
	multiSelect?: boolean;
}

// Mapping from SVG IDs to our MuscleGroup enum
const MUSCLE_MAP: Record<string, $Enums.MuscleGroup> = {
	calves: $Enums.MuscleGroup.CALVES,
	quads: $Enums.MuscleGroup.QUADS,
	abdominals: $Enums.MuscleGroup.ABDOMINALS,
	obliques: $Enums.MuscleGroup.OBLIQUES,
	hands: $Enums.MuscleGroup.HANDS,
	forearms: $Enums.MuscleGroup.FOREARMS,
	biceps: $Enums.MuscleGroup.BICEPS,
	"front-shoulders": $Enums.MuscleGroup.FRONT_SHOULDERS,
	chest: $Enums.MuscleGroup.CHEST,
	traps: $Enums.MuscleGroup.UPPER_TRAPS,
	hamstrings: $Enums.MuscleGroup.HAMSTRINGS,
	glutes: $Enums.MuscleGroup.GLUTES,
	triceps: $Enums.MuscleGroup.TRICEPS,
	lats: $Enums.MuscleGroup.LATS,
	lowerback: $Enums.MuscleGroup.LOWER_BACK,
	"traps-middle": $Enums.MuscleGroup.MIDDLE_TRAPS,
	"rear-shoulders": $Enums.MuscleGroup.REAR_SHOULDERS,
};
...
export function MuscleAnatomy({
	...
	const isSelected = (muscleId: string) => {
		const muscle = MUSCLE_MAP[muscleId];
		return muscle && selectedMuscles.includes(muscle);
	};

	const getMuscleClass = (muscleId: string) => {
		const baseClass = "bodymap cursor-pointer transition-colors";
		if (isSelected(muscleId)) {
			return `${baseClass} text-red-600`;
		}
		return `${baseClass} text-gray-400 hover:text-red-300`;
	};
	...
	return (
		<div className="flex flex-col items-center space-y-4">
			<div className="flex items-center space-x-4">
				<span
					className={`text-sm ${view === "front" ? "font-medium" : "text-gray-500"}`}
				>
					Front
				</span>
				<Switch
					...
					onCheckedChange={(checked: boolean) =>
						setView(checked ? "back" : "front")
					}
				/>
				<span
					className={`text-sm ${view === "back" ? "font-medium" : "text-gray-500"}`}
				>
					Back
				</span>
			</div>
...
interface ViewProps {
	...
	getMuscleClass: (muscleId: string) => string;
}

function FrontView({ onMuscleClick, getMuscleClass }: ViewProps) {
	return (
		<svg
			...
			className="w-full"
			role="img"
			aria-label="Front view muscle anatomy diagram"
		>
			<title>Front view muscle anatomy diagram</title>
			<g
				id="calves"
				...
				role="button"
				tabIndex={0}
				aria-label="Calves muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Quadriceps muscle group"
			>
				<path
			...
			<g
				id="abdominals"
				className={getMuscleClass("abdominals")}
				onClick={() => onMuscleClick("abdominals")}
				role="button"
				tabIndex={0}
				aria-label="Abdominals muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Obliques muscle group"
			>
				<path
			...
			<g
				id="hands"
				className={getMuscleClass("hands")}
				onClick={() => onMuscleClick("hands")}
				role="button"
				tabIndex={0}
				aria-label="Hands muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Forearms muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Biceps muscle group"
			>
				<path
			...
			<g
				id="front-shoulders"
				className={getMuscleClass("front-shoulders")}
				onClick={() => onMuscleClick("front-shoulders")}
				role="button"
				tabIndex={0}
				aria-label="Front shoulders muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Upper traps muscle group"
			>
				<path
...
function BackView({ onMuscleClick, getMuscleClass }: ViewProps) {
	return (
		<svg
			viewBox="0 0 660.46 1206.46"
			fill="none"
			...
			className="w-full"
			role="img"
			aria-label="Back view muscle anatomy diagram"
		>
			<title>Back view muscle anatomy diagram</title>
			<g
				id="traps"
				className={getMuscleClass("traps")}
				onClick={() => onMuscleClick("traps")}
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Hamstrings muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Glutes muscle group"
			>
				<path
			...
			<g
				...
				role="button"
				tabIndex={0}
				aria-label="Triceps muscle group"
			>
				<path
			...
			<g
				id="lowerback"
				className={getMuscleClass("lowerback")}
				onClick={() => onMuscleClick("lowerback")}
				role="button"
				tabIndex={0}
				aria-label="Lower back muscle group"
			>
				<path
			...
			<g
				id="traps-middle"
				className={getMuscleClass("traps-middle")}
				onClick={() => onMuscleClick("traps-middle")}
				role="button"
				tabIndex={0}
				aria-label="Traps middle muscle group"
			>
				<path
			...
			<g
				id="rear-shoulders"
				className={getMuscleClass("rear-shoulders")}
				onClick={() => onMuscleClick("rear-shoulders")}
				role="button"
				tabIndex={0}
				aria-label="Rear shoulders muscle group"
			>
				<path
